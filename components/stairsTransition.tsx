...
export default function Layout({ children, backgroundColor }: LayoutProps) {
  ...
  const anim = (
    variants: {
      ...
      enter: (i: number) => { top?: string; opacity?: number; transition: { duration: number; delay: number; ease: number[] }; transitionEnd?: { height?: string; top?: string } };
      exit: (i: number) => { height?: string; opacity?: number; transition: { duration: number; delay: number; ease: number[] } };
    },
